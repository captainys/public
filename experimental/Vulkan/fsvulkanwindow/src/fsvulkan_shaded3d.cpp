...
VkDescriptorSet FsVulkanContext::Shaded3dRenderer::CreateCurrentStateUniformBinder(Frame &frame) const
{
	return frame.CreatePerFrameUniformBinder(*this,uniformBinding[0].binding,sizeof(currentState),&currentState);
}

FsVulkanContext::Shaded3dRenderer *FsVulkanContext::RendererPool::CreateShaded3dRenderer(VkRenderPass renderPass)
{
printf("%s %d\n",__FUNCTION__,__LINE__);
	Shaded3dRenderer *renderer;
	renderer=new Shaded3dRenderer(this);
	...
	// Binding 0 Projection matrix
	{
		...
		long long int length=sizeof(Shaded3dRenderer::State);
	...
	{
		VkDescriptorSetLayoutBinding uniformBinding[1]={{}};
		uniformBinding[0].binding=2;
		uniformBinding[0].descriptorCount=YSVULKAN_SHADOWMAP_MAX_NUM_TEXTURE;
		uniformBinding[0].descriptorType=VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		uniformBinding[0].pImmutableSamplers=nullptr;
		uniformBinding[0].stageFlags=VK_SHADER_STAGE_VERTEX_BIT|VK_SHADER_STAGE_FRAGMENT_BIT;
...
////////////////////////////////////////////////////////////
	...
	auto vertShaderModule=owner->CreateShaderModule(shaded3d_vert_spv_len,shaded3d_vert_spv,"shaded3d_vert_spv");
	auto fragShaderModule=owner->CreateShaderModule(shaded3d_frag_spv_len,shaded3d_frag_spv,"shaded3d_frag_spv");
	...
	bindingDesc[2].stride=sizeof(float)*3; // Normal XYZ
	bindingDesc[2].inputRate=VK_VERTEX_INPUT_RATE_VERTEX;
	bindingDesc[3].binding=3;
	bindingDesc[3].stride=sizeof(float)*2; // Texture UV
	bindingDesc[3].inputRate=VK_VERTEX_INPUT_RATE_VERTEX;

	VkVertexInputAttributeDescription attribDesc[4]={{},{},{},{}};
	...
	renderer->attribBinding.push_back(Renderer::NORMAL3F);
